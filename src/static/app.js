    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map(email => `<li><span class="participant-email">${email}</span><button class="delete-icon" data-activity="${name}" data-email="${email}" aria-label="Delete participant">×</button></li>`).join('')}</ul>`
